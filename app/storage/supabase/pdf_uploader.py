...
This version sends an HTML email with a short hyperlink,
always BCCs two admin emails, and uses a simpler email subject
...
def _send_email_via_gmail(
    ...
    'pdf_url' will be turned into an <a> link instead of raw text.
    """
    ...
    lines.append(
        # A short hyperlink instead of the full URL
        f"<p>Your PDF is ready! "
        f'<a href="{pdf_url}" target="_blank">Open your PDF here</a>.</p>'
    )
    html_content = "\n".join(lines)

    message = MIMEText(html_content, "html")  # HTML email
    ...
    try:
        ...
        if user_email:
            ...
            _send_email_via_gmail(
                ...
                subject="Your PDF is ready!",  # No mention of report_id
                body_prefix="Hello!"
            )
        else:
